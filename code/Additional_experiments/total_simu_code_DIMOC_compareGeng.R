...
Process <- function(n,p,pi,nonzero,q,tau,B,correlation=corr){
  ...
  DIMOC=function(X,M,Y,n,p,q,pi,B,Index1,ratio=0.5){
    ...
    for (b in 1:B)
    {
      ...
      ### Construct filter for alpha: OLS * LASSO (basic, no tuning)
      
      ## ---------- D1: OLS ----------
      x1 <- data$covariate[D_1, 1]
      alpha_OLS <- apply(
        data$covariate[D_1, -1, drop = FALSE],
        2,
        function(y) coef(lm(y ~ x1))[2]
      )
      
      # ----- D2: LASSO via cv.glmnet for each mediator (basic; with dummy) -----
      x2 <- as.numeric(data$covariate[D_2, 1])
      
      # 2-column design: real X + dummy (all zeros) to avoid 1-column error
      X_alpha <- cbind(x2, rep(0, length(x2)))
      
 
      nfolds_alpha <- 10
      foldid_alpha <- sample.int(nfolds_alpha, length(x2), replace = TRUE)
      
      nlambda_alpha <- 150 
      
      alpha_LASSO <- apply(
        data$covariate[D_2, -1, drop = FALSE],
        2,
        function(y){
          fit_cv <- glmnet::cv.glmnet(
            ...
            nfolds = nfolds_alpha,
            foldid = foldid_alpha,
            nlambda = nlambda_alpha,
            parallel = FALSE
          )
          # coef: (Intercept, x2, dummy)
          as.numeric(coef(fit_cv, s = "lambda.min")[2])
        }
      )
      
      W_2 <- as.numeric(alpha_OLS) * as.numeric(alpha_LASSO)
...
print("running...")
RES0 = foreach(i=1:ns,.packages = c("MASS","lars","glmnet","ncvreg")) %dopar% Process(n,p,pi,nonzero,q,tau,B)    

saveRDS(RES0, file=paste0("compareGeng/compareGeng_pro=_",pi01,"_corr=_",corr,"_n=_",n,"_p=_",p,"_t=_",tau,"_ratio=_",ratio,"_refine=_",B,"_mediation.RData"))
